...
class PolicyWarper:
    ...
    def _normalize_task_descriptions(task_descriptions):
        """Coerce the env's per-env instruction container into a plain list of strings.

        The vector env returns one instruction PER ENV. The old code took element [0] and
        applied it to the whole batch, so with n_envs > 1 every env was driven by env 0's
        instruction. Invisible on the 6 base suites (1 instruction each) but catastrophic on
        the 18 PosttrainPnPNovel suites, where 10 different objects share one scene and only
        the sentence says which to pick. Port of upstream #401 "Fix RoboCasa batched instructions".
        """
    ...
    def step(self, observations, **kwargs) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        # Keep ALL per-env instructions, not just [0] -- see _normalize_task_descriptions.
        ...
        # NO_STATE=1 omits the "state" key entirely. QwenGR00T.forward does
        #   state = [e["state"] for e in examples] if "state" in examples[0] else None
        # so omitting it bypasses action_model.state_encoder. That encoder sits at random
        # initialisation in every checkpoint we have (ours AND the released upstream one --
        # verified: weights match 1/sqrt(fan_in) init bounds exactly), yet eval always feeds
        # it 58 dims of real proprioception. Untrained weights on real input inject noise
        # into the action pathway, which would cap every model equally.
        _no_state = os.environ.get("NO_STATE", "0") not in ("0", "", "false", "False")
        # DEBUG_LANG=1 prints the actual prompt string once. `_select_instruction` only
        # unpacks `list`; a tuple falls through and would be stringified into "lang",
        # which would silently corrupt every prompt when n_envs > 1.
        if os.environ.get("DEBUG_LANG") and not getattr(self, "_lang_dumped", False):
            self._lang_dumped = True
            print(
                f"[lang-debug] raw_type={type(instructions).__name__} raw={instructions!r} "
                f"batch_size={batch_size} selected_b0={self._select_instruction(instructions, 0)!r}",
                flush=True,
            )
        ...
        # Inference sampler is overridable for diagnostics. Upstream's config.yaml records
        # num_inference_timesteps: 4, while PolicyWarper defaults to num_ddim_steps=10.
        _use_ddim = self.use_ddim
        if os.environ.get("USE_DDIM") is not None:
            _use_ddim = os.environ["USE_DDIM"] not in ("0", "", "false", "False")
        _num_ddim = int(os.environ.get("NUM_DDIM_STEPS", self.num_ddim_steps))

        vla_input = {
            "examples": examples,
            "do_sample": False,
            "use_ddim": _use_ddim,
            "num_ddim_steps": _num_ddim,
        }
