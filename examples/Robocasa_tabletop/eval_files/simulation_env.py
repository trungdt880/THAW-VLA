...
class SimulationInferenceEnv:
    ...
    # Which env video key gets mapped onto `video.ego_view` (what the policy consumes).
    # The env exposes several ego-view variants and the original hot fix hard-coded the
    # background-cropped one. Training data has a single plain `observation.images.ego_view`,
    # and it has never been verified that the two are the same image distribution — a mismatch
    # would depress every reported success rate. Override with EGO_VIEW_KEY to A/B it.
    # Default preserves the historical behaviour so existing results stay reproducible.
    _EGO_VIEW_KEY = os.environ.get("EGO_VIEW_KEY", "video.ego_view_bg_crop_pad_res256_freq20")
    _logged_keys = False

    def get_action(self, observations: Dict[str, Any]) -> Dict[str, Any]:
        """Get action from the model based on observations."""
        if not SimulationInferenceEnv._logged_keys:
            SimulationInferenceEnv._logged_keys = True
            vid = sorted(k for k in observations if k.startswith("video."))
            print(f"[ego-view] env provides video keys: {vid}", flush=True)
            print(f"[ego-view] mapping '{self._EGO_VIEW_KEY}' -> 'video.ego_view'", flush=True)
            if self._EGO_VIEW_KEY not in observations:
                print(
                    f"[ego-view] WARNING: '{self._EGO_VIEW_KEY}' NOT in observations; "
                    f"policy will see whatever 'video.ego_view' already holds",
                    flush=True,
                )
        if self._EGO_VIEW_KEY in observations:
            observations["video.ego_view"] = observations.pop(self._EGO_VIEW_KEY)
