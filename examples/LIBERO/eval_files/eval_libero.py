...
class Args:
    ...
    # --- wandb (optional): attach to the training run and log success rate ---
    # If wandb_project is set, this process joins wandb run `wandb_run_id` in
    # "shared" mode (multi-writer) and logs eval/<suite>/success_rate against
    # eval/ckpt_step. Requires the training run to have started with --wandb_shared.
    wandb_project: str = ""
    wandb_entity: str = ""
    wandb_run_id: str = ""        # = training run_id (the shared wandb run to join)
    wandb_ckpt_step: int = -1     # checkpoint step, used as the eval x-axis


def _log_eval_to_wandb(args: "Args", suite: str, success_rate: float, n_episodes: int) -> None:
    """Join the training wandb run (shared mode) and log this suite's success rate."""
    ...
    try:
        ...
        run.log(
            {
                f"eval/{suite}/success_rate": success_rate,
                f"eval/{suite}/episodes": n_episodes,
                "eval/ckpt_step": args.wandb_ckpt_step,
            }
        )
        run.finish()
    except Exception as e:  # eval must never crash because of wandb
        logging.warning(f"[wandb] eval logging failed (non-fatal): {e}")
...
def eval_libero(args: Args) -> None:
    ...
    _log_eval_to_wandb(args, args.task_suite_name, total_success_rate, total_episodes)
