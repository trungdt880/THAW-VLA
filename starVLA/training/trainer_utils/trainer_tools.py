...
def build_param_lr_groups(model, cfg):
    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            for attr in module_name.split("."):
                module = getattr(module, attr)
            # filter out frozen parameters: both those frozen by `freeze_modules`
            # pattern AND any param with requires_grad=False (e.g. a peft LoRA-wrapped
            # LM whose base weights are frozen, or a frozen in-graph teacher). Such
            # params must NOT be handed to the optimizer / sharded by DeepSpeed.
            params = [
    ...
    # and any param with requires_grad=False — e.g. a frozen in-graph teacher submodule that
    # must NOT be handed to the optimizer / sharded by deepspeed)
    other_params = [
