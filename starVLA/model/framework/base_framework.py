    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            # Training-only modules dropped at inference (e.g. the online V-JEPA REPA teacher,
            # STARVLA_INFERENCE=1) leave EXTRA keys in the checkpoint but miss NONE of the model's
            # own keys. That is safe: load non-strict and continue.
            droppable_prefixes = ("online_repa_teacher.",)
            only_droppable_extra = (
                not missing_keys
                and unexpected_keys
                and all(k.startswith(droppable_prefixes) for k in unexpected_keys)
            )
            if only_droppable_extra:
                logger.warning(
                    f"Ignoring {len(unexpected_keys)} training-only key(s) "
                    f"(prefix {droppable_prefixes}) at inference; loading non-strict."
                )
