...
class _QWen3_5_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        # Default stays "sdpa" -- only a config that explicitly asks for flash_attention_2
        # changes behaviour, so no existing run is affected silently.
        #
        # The line removed here was an unconditional `attn_implementation = "sdpa"` that
        # discarded the config value and made the flash_attn fallback below unreachable.
        # It came from upstream b6ab70e (#315, 2026-05-19); the Dec-2025 code that produced
        # the released RoboCasa checkpoints passed attn_implementation="flash_attention_2".
        # Measured on Qwen3.5-0.8B (bs16, bf16, grad-ckpt, fwd+bwd): sdpa 817 ms/step vs
        # FA2 743 ms/step = 1.10x. Modest, because GR1 batches are only ~1% padding, so
        # FA2's varlen packing has little to exploit. Both are EXACT attention -- they differ
        # in floating-point ordering, not in what they compute.
        ...
        print(f"[QWen3_5] attn_implementation = {attn_implementation}")
