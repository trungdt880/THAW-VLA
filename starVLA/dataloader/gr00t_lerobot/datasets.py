...
# FastWAM distillation cache kinds: maps a logical channel to its data_cfg keys and the
# sample-dict keys it populates. Channel-1 (repa) is always available; Channel-2 (future)
# is opt-in via `fastwam_future_cache`.
_FASTWAM_KINDS = {
    "repa": {
        "path_key": "fastwam_target_cache",
        "mode_key": "fastwam_mode",
        "mode_default": "pooled",
        "sample_key": "fastwam_target",
        "valid_key": "fastwam_valid",
    },
    "future": {
        "path_key": "fastwam_future_cache",
        "mode_key": "fastwam_future_mode",
        "mode_default": "future",
        "sample_key": "fastwam_future_target",
        "valid_key": "fastwam_future_valid",
    },
}

# --- ONLINE V-JEPA2-AC REPA teacher input construction (Channel-1) ---
# Mirrors tools/vjepa_precompute_targets.py constants exactly. Clips/states/actions are
# built in the DataLoader worker and consumed by VJepaOnlineTeacher in the model forward.
_VJEPA_ONLINE_FRAMES = 8        # dataset_fpcs (max_num_frames)
_VJEPA_ONLINE_FPS = 4           # data.fps
_VJEPA_ONLINE_VIDEO_FPS = 20.0  # LIBERO lerobot video fps
_VJEPA_ONLINE_CROP = 256        # crop_size (== patch grid 16x16)
# ImageNet normalization (transforms.make_transforms default), in 0-255 space.
_VJEPA_IMAGENET_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32) * 255.0
_VJEPA_IMAGENET_STD = np.array([0.229, 0.224, 0.225], dtype=np.float32) * 255.0


def _vjepa_frames_to_clip(frames: np.ndarray, crop: int = _VJEPA_ONLINE_CROP) -> np.ndarray:
    """frames [T,H,W,3] uint8 -> normalized clip np.float32 [3,T,crop,crop].

    Deterministic center transform: resize each frame to crop_size and ImageNet-normalize
    (LIBERO frames are already 256px so this is effectively identity + normalize).
    """
    out = []
    for f in frames:
        img = Image.fromarray(f).resize((crop, crop))
        arr = np.asarray(img, dtype=np.float32)  # [H,W,3]
        arr = (arr - _VJEPA_IMAGENET_MEAN) / _VJEPA_IMAGENET_STD
        out.append(arr.transpose(2, 0, 1))  # [3,H,W]
    return np.stack(out, axis=1).astype(np.float32)  # [3,T,H,W]


def _vjepa_poses_to_diffs(poses: np.ndarray) -> np.ndarray:
    """Port of DROIDVideoDataset.poses_to_diffs: poses [T,7] -> diffs [T-1,7]."""
    ...
    angle_diff = [Rotation.from_matrix(m).as_euler("xyz", degrees=False) for m in angle_diff]
...
def _vjepa_states_actions(state8_window: np.ndarray):
    """state8_window [T,8] LIBERO observation.state -> (states7 [T,7], actions7 [T,7]).

    7-d state = [cartesian(x,y,z,roll,pitch,yaw), gripper] (col 6 is pad). Actions are
    poses_to_diffs (T-1 rows), front-padded with a zero row to length T so the predictor's
    per-frame interleave is shape-consistent.
    """
    cart = state8_window[:, 0:6].astype(np.float32)
    grip = state8_window[:, 7:8].astype(np.float32)
    states7 = np.concatenate([cart, grip], axis=1)  # [T,7]
    diffs = _vjepa_poses_to_diffs(states7)  # [T-1,7]
    actions7 = np.concatenate([np.zeros((1, 7), dtype=np.float32), diffs], axis=0)  # [T,7]
    return states7.astype(np.float32), actions7.astype(np.float32)
...
class LeRobotSingleDataset(Dataset):
    ...
    def _pack_sample(self, data: dict, trajectory_id=None, base_index=None) -> dict:
        ...
        # FastWAM distillation targets (training-only), precomputed offline over THESE
        # frames and keyed by (traj_id, base_index). Channel-1 (REPA, frame-0 hidden) and
        # optional Channel-2 (future Wan-VAE latents).
        if trajectory_id is not None and base_index is not None:
            for kind in ("repa", "future"):
                if not self._fastwam_cache_enabled(kind):
                    continue
                cfg = _FASTWAM_KINDS[kind]
                target, valid = self._fastwam_cache_lookup(trajectory_id, base_index, kind)
                if target is not None:
                    sample[cfg["sample_key"]] = target  # np.float16 [target_dim]
                    sample[cfg["valid_key"]] = bool(valid)

        # ONLINE V-JEPA2-AC REPA teacher (Channel-1, in-training-loop). Opt-in via
        # datasets.vla_data.online_repa_teacher == "vjepa". Attaches the RAW teacher
        # inputs (clips/states/actions) -- the GPU forward happens in the model. This
        # decode runs in the DataLoader worker (parallel; no offline precompute / mmap).
        if (
            trajectory_id is not None
            and base_index is not None
            and self.data_cfg is not None
            and self.data_cfg.get("online_repa_teacher", None) == "vjepa"
        ):
            self._attach_online_repa_inputs(sample, trajectory_id, base_index)

        return sample

    # ------------------------------------------------------------------
    # Online V-JEPA2-AC REPA teacher inputs (Channel-1).
    # Ported from tools/vjepa_precompute_targets.py:fetch_teacher_inputs, but emits
    # RAW numpy inputs (collate is passthrough; the model moves them to device and runs
    # the frozen teacher in-graph). On any failure we attach zero clips + valid=False so
    # the batch shapes stay uniform (T=8 is fixed -> clips batch cleanly).
    # ------------------------------------------------------------------
    def _attach_online_repa_inputs(self, sample: dict, trajectory_id, base_index) -> None:
        n_cam = int(self.data_cfg.get("repa_n_cam", 2))
        T = _VJEPA_ONLINE_FRAMES
        crop = _VJEPA_ONLINE_CROP
        try:
            clips, states, actions, clamped = self._build_online_repa_inputs(
                trajectory_id, base_index, n_cam
            )
            sample["repa_online_clips"] = clips          # list[n_cam] of np.float32 [3,T,crop,crop]
            sample["repa_online_states"] = states        # np.float32 [T,7]
            sample["repa_online_actions"] = actions      # np.float32 [T,7]
            sample["repa_online_valid"] = bool(not clamped)
        except Exception as exc:  # noqa: BLE001 - never let one bad decode kill the worker
            import warnings
            warnings.warn(f"online_repa_teacher inputs failed for ({trajectory_id},{base_index}): {exc}", stacklevel=2)
            sample["repa_online_clips"] = [
                np.zeros((3, T, crop, crop), dtype=np.float32) for _ in range(n_cam)
            ]
            sample["repa_online_states"] = np.zeros((T, 7), dtype=np.float32)
            sample["repa_online_actions"] = np.zeros((T, 7), dtype=np.float32)
            sample["repa_online_valid"] = False

    def _build_online_repa_inputs(self, trajectory_id, base_index, n_cam: int):
        """Build (per-camera clips, states, actions, clamped) for one step.

        Returns:
          clips:   list[n_cam] of np.float32 [3,T,crop,crop]  (ImageNet-normalized)
          states:  np.float32 [T,7]
          actions: np.float32 [T,7]
          clamped: bool (window ran past the episode end -> mark invalid)
        """
        from math import ceil

        traj_df = self.get_trajectory_data(trajectory_id)
        state8 = np.stack(traj_df["observation.state"].values)  # [Tfull, 8]
        max_index = state8.shape[0] - 1

        # 8-frame window anchored at frame 0 == base_index, stride ceil(VIDEO_FPS/VJEPA_FPS).
        fstp = int(ceil(_VJEPA_ONLINE_VIDEO_FPS / _VJEPA_ONLINE_FPS))  # 5 for LIBERO
        raw = [base_index + k * fstp for k in range(_VJEPA_ONLINE_FRAMES)]
        clamped = raw[-1] > max_index
        idxs = [min(i, max_index) for i in raw]
        ts = [i / _VJEPA_ONLINE_VIDEO_FPS for i in idxs]

        vkeys = self.modality_keys["video"][:n_cam]
        clips = []
        for vk in vkeys:
            key = vk.replace("video.", "")
            vpath = str(self.get_video_path(trajectory_id, key))
            frames = get_frames_by_timestamps(
                vpath, ts, self.video_backend, self.video_backend_kwargs
            )  # [T,H,W,3] uint8
            clips.append(_vjepa_frames_to_clip(frames, crop=_VJEPA_ONLINE_CROP))

        state_window = state8[idxs, :]  # [T,8]
        states7, actions7 = _vjepa_states_actions(state_window)
        return clips, states7, actions7, clamped

    # ------------------------------------------------------------------
    # FastWAM distillation cache (training-only; lazy per-worker mmap).
    # `kind` selects Channel-1 (repa) vs Channel-2 (future) config keys + state.
    # ------------------------------------------------------------------
    def _fastwam_cache_enabled(self, kind: str = "repa") -> bool:
        if getattr(self, "data_cfg", None) is None:
            return False
        return bool(self.data_cfg.get(_FASTWAM_KINDS[kind]["path_key"], None))

    def _fastwam_cache_open(self, kind: str) -> dict:
        """Lazily open a targets memmap (fork-safe; one handle per DataLoader worker)."""
        states = getattr(self, "_fastwam_cache_states", None)
        if states is None:
            states = {}
            self._fastwam_cache_states = states
        if kind in states:
            return states[kind]
        import os

        from .fastwam_cache import (
            VALID_FILENAME,
            open_targets_memmap,
            read_meta,
        ...
        cfg = _FASTWAM_KINDS[kind]
        cache_path = self.data_cfg.get(cfg["path_key"])
        variant = self.data_cfg.get("fastwam_variant", "fastwam")
        tap_layer = int(self.data_cfg.get("fastwam_tap_layer", 15))
        mode = str(self.data_cfg.get(cfg["mode_key"], cfg["mode_default"]))
        cache_dir = resolve_dataset_cache_dir(cache_path, self.dataset_name, variant, tap_layer, mode)
        meta = read_meta(cache_dir)
        validate_meta(
            meta,
            expected_steps_hash=steps_hash(self.all_steps),
            expected_n_samples=len(self.all_steps),
        )
        if meta.get("partial", False):
            raise CacheIntegrityError(
                f"REPA cache at {cache_dir} is marked partial (built with --limit "
                f"{meta.get('partial_limit')} of {meta.get('n_samples')} rows). Most targets are "
                f"absent; training on it silently drops the alignment signal. Rebuild without --limit."
            )
        targets = open_targets_memmap(cache_dir, mode="r")
        valid_path = os.path.join(cache_dir, meta.get("valid_file", VALID_FILENAME))
        if os.path.exists(valid_path):
            valid = np.load(valid_path, mmap_mode="r")
        else:
            # Absent valid.npy means the precompute never finalised. Treating it as
            # "all valid" makes a zero-filled memmap look like a working cache, so refuse.
            raise CacheIntegrityError(
                f"REPA cache at {cache_dir} has no {os.path.basename(valid_path)}: the precompute "
                f"did not finalise. Re-run scripts/01_precompute_teacher.sh; training on this cache "
                f"would silently contribute zero gradient."
            )
        key_to_row = {(str(t), int(b)): i for i, (t, b) in enumerate(self.all_steps)}
        state = {
            "targets": targets,
            "valid": valid,
            "key_to_row": key_to_row,
            "target_dim": int(meta["target_dim"]),
            "grid_hw": (meta.get("grid_h"), meta.get("grid_w")),
            "n_future": meta.get("n_future"),
    ...
    def _fastwam_cache_lookup(self, trajectory_id, base_index, kind: str = "repa"):
        """Return (target_fp16 [target_dim] copy, valid_bool) or (None, False) on miss."""
        state = self._fastwam_cache_open(kind)
        row = state["key_to_row"].get((str(trajectory_id), int(base_index)), None)
        if row is None:
            return None, False
        target = np.asarray(state["targets"][row]).copy()  # detach from mmap
        valid = True if state["valid"] is None else bool(state["valid"][row])
        return target, valid
...
class CacheIntegrityError(RuntimeError):
    """Teacher-cache corruption. Deliberately NOT retryable.

    __getitem__ retries on generic exceptions at a fresh random index; a corrupt cache in one
    dataset of a mixture would therefore be retried into a healthy dataset and swallowed, silently
    dropping that dataset's samples while `repa_valid_frac` still reads 1.0 (it is computed only
    over samples that reached the batch). These must abort the run.
    """
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Never retry cache corruption -- retrying lands in a healthy dataset and hides it.
                raise
            except Exception as e:
                last_exception = e
